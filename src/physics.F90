...
!===============================================================================
  ...
  subroutine elastic_scatter(i_nuclide, rxn, kT, E, uvw, mu_lab, wgt, p)
    integer, intent(in)     :: i_nuclide
    ...
    real(8) :: kT_val, alpha_min, alpha_max, beta_min, beta_max, rand_temp1
    real(8) :: rand_temp2
    real(8) :: E_min, E_max, mu_min, mu_max, local_E, norm_const
    real(8) :: sample_limit, E_candidate, mu_candidate, interpolated_Sab
    real(8) :: alpha_candidate, beta_candidate, rejection_check, temp_real
    integer :: loop_counter, max_loops = 1E6, i, j
    logical :: accepted
    logical, parameter :: detailed_limit_output = .true.
    ...
    if (nuclides(i_nuclide) % name == "C0" .and. E < FREE_GAS_THRESHOLD * kT ) then
       ...
       ! Set the max and min alpha, beta
       alpha_min = (E_min + E - 2.0_8 * mu_max * sqrt(E_min*E)) / (awr*kT)
       alpha_max = (E_max + E - 2.0_8 * mu_min * sqrt(E_max*E)) / (awr*kT)
       beta_min = (E_min - E) / kT
       beta_max = (E_max - E) / kT

       ! Sample limit is always the same value
       sample_limit = 100.000_8

       ! Now we randomly pick an E' and mu, and check via rejection
       ! sampling
       loop_counter = 0
       accepted = .false.
       do while (.not. accepted)
         ...
         ! Sample E' and mu uniformly
         rand_temp1 = prn()
         rand_temp2 = prn()
!        call random_number(rand_temp1)
!        call random_number(rand_temp2)
         E_candidate = E_min + rand_temp1 * (E_max - E_min)
         mu_candidate = mu_min + rand_temp2 * (mu_max - mu_min)

         ! Convert these into alpha and beta
         alpha_candidate = (E_candidate + E - 2.0_8 * mu_candidate *sqrt(E_candidate*E)) &
                           / (awr*kT)
         beta_candidate = (E_candidate - E) / kT

         ! Sample the number we'll be rejection sampling with
         rejection_check = prn()
!        call random_number(rejection_check)

         ! See what the Sab corresponding to this would be
         temp_real = (beta_candidate + alpha_candidate)**2 / (4.0_8 * &
                      alpha_candidate)
         interpolated_Sab = sqrt(E_candidate) * exp(-1.0_8 * temp_real) / &
                               sqrt(4.0_8 * PI * alpha_candidate)
         ! Accept if we are within the criteria
         if (rejection_check <= (interpolated_Sab / sample_limit) ) then
           if ((interpolated_Sab / sample_limit) > 1.0_8) then
             print *, 'ERROR: The free gas sample limit is not high enough without'
             print *, 'a weight correction to compensate'
             print *, '       We are currently getting a ratio of ', &
                      (interpolated_Sab / sample_limit)
             if (detailed_limit_output) then
               print *, 'The following are the details of this scattering event'
               print *, 'Particle ID: ', p % id
               print *, 'Particle num collisions: ', p % n_collision
               print *, 'Incident neutron energy: ', E
               print *, 'Particle u, v, w: ', uvw
               print *, 'Rejection limit: ', sample_limit
               print *, 'Sampling loop attempt number: ', loop_counter
               print *, 'Candidate E_prime: ', E_candidate
               print *, 'Candidate mu: ', mu_candidate
               print *, 'Candidate alpha: ', alpha_candidate
               print *, 'Candidate beta: ', beta_candidate
               print *, 'Candidate S(a,b): ', interpolated_Sab
               print *, 'Sampled E_prime random number: ', rand_temp1
               print *, 'Sampled mu random number: ', rand_temp2
               print *, 'Sampled rejection random number: ', rejection_check
               print *, '**************'
             end if
           end if
           accepted = .true.
         ...
         if (loop_counter > max_loops) then
           ...
           print *, 'WARNING: encountered maximum number of rejection sampling'
           print *, 'trials for S(a,b)'
           accepted = .true.
         end if

       end do ! End of while loop for acceptance

       ! We finally have outgoing energy and angle. Transfer these to the output
       ! parameters
       E = E_candidate
       mu_lab = mu_candidate
       ! change direction of particle
